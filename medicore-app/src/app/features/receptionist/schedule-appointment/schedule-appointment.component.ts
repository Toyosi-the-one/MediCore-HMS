...
import {
  ...
  DOCTORS_BY_DEPT,
  ALL_TIME_SLOTS,
} from '../../../core/data/appointment.config';
...
export class ScheduleAppointmentComponent implements OnInit {
  ...
  isSlotDisabled(slot: string) {
    return this.isSlotTaken(slot) || this.isSlotPast(slot);
  }
  isSlotSelected(slot: string) {
    return this.form.get('timeSlot')?.value === slot;
  }
        ...
        if (this.editId()) {
          ...
          if (!old) {
            this.toastService.error(
              'Booking Failed',
              'Original appointment data missing. Please try again.',
            );
            return;
          }
          ...
          this.toastService.success(
            ...
            `Rescheduled for ${val.timeSlot} on ${val.date}`,
          );
        } else {
          await this.receptionistService.scheduleAppointment(payload);
          this.toastService.success(
            'Appointment Scheduled',
            `Booked for ${val.timeSlot} on ${val.date}`,
          );
        }
