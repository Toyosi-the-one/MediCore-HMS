...
export class PatientRegistrationComponent {
  ...
  private isCurrentStepValid(): boolean {
    ...
    if (step === 3)
      fields = ['allergies', 'medicalHistory', 'emergencyContactName', 'emergencyContactPhone'];
  ...
  async submit() {
    if (this.form.valid) {
      ...
      try {
        ...
        const newId = await Promise.race([this.receptionistService.registerPatient(v), timeout]);
        this.newlyRegisteredPatientId.set(newId);
        this.submitted.set(true);
        this.toastService.success(
          'Registration Complete',
          `${v.firstName} ${v.lastName} was registered successfully.`,
        );
      } catch (err: any) {
  ...
  scheduleAppointment() {
    ...
    if (id) {
      this.router.navigate(['/user/appointments/new'], { queryParams: { patientId: id } });
    } else {
